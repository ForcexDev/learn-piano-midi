...
export const PianoKeyboard: React.FC<PianoKeyboardProps> = ({
  ...
  return (
    <div className="w-full flex flex-col items-center select-none py-1">
      <div className="relative w-full bg-slate-950 p-2 sm:p-3 rounded-2xl shadow-2xl border border-slate-800/90 overflow-hidden">
        {/* Render 36 White Keys in full width flex container */}
        <div className="w-full flex relative h-48 sm:h-56 md:h-64 lg:h-72">
          {whiteKeys.map((midiNumber) => {
            ...
            return (
              <button
                ...
                className={`flex-1 relative rounded-b-xl flex flex-col justify-end items-center pb-2.5 sm:pb-4 transition-all duration-75 origin-top ${keyBgClass} ${borderClass}`}
              >
                {/* Middle C Indicator Marker */}
                {isMiddleC && (
                  <div className="absolute top-2 left-1/2 -translate-x-1/2 flex flex-col items-center">
                    <span className="w-2.5 h-2.5 rounded-full bg-blue-600 ring-2 ring-blue-300 shadow-md" title="Middle C (C4)" />
                    <span className="text-[9px] font-extrabold text-blue-600 mt-0.5">C4</span>
                  </div>
                )}

                {/* Octave Marker for other C keys */}
                {!isMiddleC && isCKey && (
                  <span className="absolute top-2 left-1/2 -translate-x-1/2 text-[9px] font-bold text-slate-400">
                    C{noteObj.octave}
                  </span>
                )}
                ...
                <div className="flex flex-col items-center z-10 pointer-events-none">
                  <span className={`text-[10px] sm:text-xs md:text-sm font-extrabold tracking-tight ${
                  ...
                  {showSolfegeBridge && settings.showSolfegeBridge && noteObj.solfegeEquivalent && (
                    <span className={`text-[8px] sm:text-[10px] font-medium leading-none opacity-80 ${
        ...
        {/* Render 25 Overlay Black Keys aligned EXACTLY on white key seams */}
        <div className="absolute top-2 sm:top-3 left-2 sm:left-3 right-2 sm:right-3 h-32 sm:h-36 md:h-40 lg:h-44 pointer-events-none">
          {whiteKeys.map((midiNumber, index) => {
            ...
            // Mathematically exact seam alignment:
            // The boundary seam between white key `index` and `index+1` is at `((index + 1) / totalWhite) * 100%`
            const seamPercent = ((index + 1) / totalWhite) * 100;
            const keyWidthPercent = (100 / totalWhite) * 0.65; // Black key is 65% width of a white key
            ...
            return (
              <button
                ...
                className={`absolute top-0 h-32 sm:h-36 md:h-40 lg:h-44 rounded-b-lg flex flex-col justify-end items-center pb-2.5 sm:pb-3 pointer-events-auto transition-all duration-75 z-20 origin-top ${keyBgClass} ${borderClass}`}
              >
                <span className="text-[8px] sm:text-[10px] md:text-xs font-bold leading-tight tracking-tight select-none">
                  {noteObj.displayName}
                </span>
              </button>
            );
